...
=head1 OPÇÕES DE ATRIBUTO

Use a função C<has> para declarar um atributo:
...
Isto diz que todos os objetos C<Person> tem um atributo opcional 
de leitura-escrita "first_name".

=head2 Leitura-Escrita vs. Apenas-Leitura

As opções informada para o C<has> define as propriedades do 
atributo. Há muitas opções, mas na forma mais simples, você só 
precisa definir C<is>, que pode ser tanto C<rw> (leitura-escrita) 
ou C<ro> (somente leitura). Quando um atributo é C<rw>, você 
pode alterar o seu valor através do assessor. Quando um atributo 
é C<ro>, você pode apenas ler o valor atual do atributo.

Na verdade, você pode até omitir C<is>, mas com isto você terá um atributo 
que não tem nenhum assessor. Isso pode ser útil com outras opções de atributo, 
como C<handles>. No entanto, se o seu atributo I<não> gera assessores, 
o Moose irá emitir um aviso, porque normalmente significa que o programador 
esqueceu de dizer o atributo é somente leitura ou leitura-gravação. Se 
você realmente quer dizer que não têm assessores, você pode silenciar 
este aviso trocando C<is> por C<bare>.

=head2 Accessor methods

Cada atributo tem um ou mais métodos de acesso. Um assessor lhe permite ler 
e escrever o valor do atributo para um objeto.

Por padrão, o método de assessor tem o mesmo nome do atributo. Se 
você declarou seu atributo como C<ro> então seu assessor será somente leitura. 
Se você declarou como leitura-escrita, você obtém um assessor de leitura-escrita. 
Simples assim.

Dada a classe C<Person> exemplo acima, temos agora o único 
assessor C<first_name> que pode ler ou escrever no atributo de 
valor C<first_name> do objeto C<Person>.

Se quiser, você também pode especificar explicitamente o nome do 
método a ser usado para leitura e escrita de um atributo. Isto é 
particularmente útil quando você quiser que um atributo seja lido 
publicamente, mas escrito apenas em privado. Por exemplo:
...
Isto pode ser útil se o C<weight> é calculado com base em outros métodos. 
Por exemplo, toda vez que o C<eat> método é chamado, pode-se ajustar o 
C<weight>. Isso nos permite esconder os detalhes de implementação de 
mudanças de C<weight>, mas ainda fornecer o valor do C<weight> para 
os usuários da classe.

Algumas pessoas podem preferir os métodos distintos para a leitura e escrita. 
Em I<Perl Best Practices>, Damian Conway recomenda que os métodos de 
leitura começam com "get_" e aos métodos de escrita começam com "set_".

Podemos fazer exatamente isso, fornecendo nomes, tanto para os métodos
C<reader> e C<writer>:
...
Se você está pensando que fazer isto frequentemente é um trabalho 
tedioso, você está certo! Felizmente, Moose fornece um sistema de 
extensão poderosa que lhe permite substituir o padrão para a convenções 
de nomenclatura. Veja L<Moose::Manual::MooseX> para mais detalhes.

=head2 Predicado e métodos mais claro
